...
class SNNAP:
    ...
    def __init__(self, clip_runtime=True, feature_selection='chi-squared', top_n=3, k_neighbours=60):
        ...
        self._scaler = MaxAbsScaler(max_abs=1)
        self._runtime_scaler = StandardScaler()
        self._models = []
        self._rfr_params = {'n_estimators': 100, 'criterion':'mse', 'max_depth':None, 'min_samples_split':2}
    ...
    def fit(self, scenario: ASlibScenario, fold: int, num_instances: int):
        ...
        self._models = [RandomForestRegressor(random_state=fold, **self._rfr_params) for alg in range(self._num_algorithms)]
        ...
        self._index = BallTree(performances, leaf_size=30, metric='pyfunc', func=SNNAP._top_n_jaccard, metric_params={'top_n': self._top_n})
    ...
    def predict(self, features, instance_id: int):
        ...
        predicted = np.asarray([model.predict(features) for model in self._models]).reshape(1,-1)
        neighbour_idx = np.squeeze(self._index.query(predicted, self._k_neighbours, return_distance=False))
    ...
    def _preprocess_scenario(self, scenario: ASlibScenario, features, performances):
        ...
        if self._clip_runtime:
            performances = np.clip(performances, a_min=np.NINF, a_max=scenario.algorithm_cutoff_time)
